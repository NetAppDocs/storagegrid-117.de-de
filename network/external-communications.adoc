...
NOTE: Diese Liste enthält keine Ports, die als konfiguriert werden können link:../admin/configuring-load-balancer-endpoints.html["Load Balancer-Endpunkte"] Oder verwendet für link:../monitor/configuring-syslog-server.html["Syslog-Server"].
...
Alle Nodes
 a| 
Bietet den Multicast-DNS-Dienst (mDNS), der für IP-Änderungen im gesamten Netzwerk und für die Ermittlung des primären Admin-Knotens während der Installation, Erweiterung und Wiederherstellung verwendet wird.
