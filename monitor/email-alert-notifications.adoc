...
NOTE: Wenn Sie die Warnbenachrichtigungen testen, müssen Sie sich bei jedem Admin-Knoten anmelden, um die Verbindung zu überprüfen. Dies steht im Gegensatz zum Testen von AutoSupport-Meldungen und älteren Alarmmeldungen, bei denen alle Admin-Knoten die Test-E-Mail senden.
