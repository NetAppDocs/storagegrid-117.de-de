...
Es gibt zwei Laufwerkstypen: SSD und HDD. Sie müssen den verwenden link:upgrading-drive-firmware-using-santricity-system-manager-offline.html["Offline-Methode"] Um die Firmware auf den SSDs zu aktualisieren (z. B. SSD-Laufwerke im SG6060). Sie können entweder die Online- oder die Offline-Methode verwenden, um die Firmware auf HDDs zu aktualisieren.
...
[NOTE]
====
Ein Online-Upgrade kann mehrere Stunden länger dauern als ein Offline-Upgrade.

Sie müssen den verwenden link:upgrading-drive-firmware-using-santricity-system-manager-offline.html["Offline-Methode"] Um die Firmware auf SSDs zu aktualisieren.
