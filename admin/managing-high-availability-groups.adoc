...
* Eine HA-Gruppe, die nur SG100- oder SG1000-Appliances und VMware-basierte Software-Nodes enthält, kann für hochverfügbare Verbindungen bereitstellen link:../admin/manage-s3-select-for-tenant-accounts.html["S3-Mandanten, die S3 Select nutzen"]. HA-Gruppen werden empfohlen, wenn S3 Select verwendet wird, jedoch nicht erforderlich.
