...
Wenn Sie Speicherknoten einer StorageGRID-Appliance bereitstellen müssen, finden Sie weitere Informationen unter link:../installconfig/deploying-appliance-storage-node.html["Appliance-Storage-Node implementieren"].
...
.Bevor Sie beginnen
* Sie haben die Vorgehensweise überprüft link:planning-and-preparation.html["Installation planen und vorbereiten"]Und Sie verstehen die Anforderungen an Software, CPU und RAM sowie Storage und Performance.
...
IMPORTANT: Weisen Sie die in den Endpunkten des Load Balancer verwendeten Ports nicht neu zu.
